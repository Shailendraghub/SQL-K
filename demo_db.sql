...
-- 1. findthe top 3 product id based of the number of quantity sold..
-- 2. Get the order date ,the product id, product name, and the order id which has been in sold in the multiples of 2. 
-- 3. Get the brand name, the order value, the product name for each product 
-- sold. 
-- 4. Get the avaerge order value for each order which has been placed after half an hour.
-- 5. get the order id with number of quantity sold the month and the total price for each brand. 

select product_id, sum(quantity) as total from order_lines group by product_id order by total desc limit 3 ;

select o.order_date, p.product_id,p.product_name, ol.order_id from orders o inner join
order_lines ol on ol.order_id = o.id inner join products p 
on ol.product_id = p.product_id where ol.quantity %2=0;


select b.brand, o.order_value,p.product_name from brands b 
